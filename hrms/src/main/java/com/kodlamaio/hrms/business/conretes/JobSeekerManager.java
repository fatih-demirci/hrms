...
public class JobSeekerManager implements JobSeekerService{
	...
	@Autowired
	public JobSeekerManager(JobSeekerDao jobSeekerDao,PersonValidationService personValidation,MailValidService mailValidateService,MailValidationService mailValidationService) {
	...
	public Result add(@RequestBody JobSeeker jobSeeker) {
		...
		if(!jobSeeker.getMember().getEMail().isEmpty()) {
			eMailIsEmpty=false;
			if(mailValidateService.mailIsValid(jobSeeker.getMember().getEMail())) {
				mailIsValid = true;
		...
		/*System.out.println("Name "+nameIsEmpty+ " "+
		"Last name "+lastnameIsEmpty+
		" nationalIdentityIsEmpty "+nationalIdentityIsEmpty+
		" nationalIdentiyisValid "+nationalIdentiyisValid+
		" dateOfBirthIsEmpty "+dateOfBirthIsEmpty+
		...
		if(!nameIsEmpty&&!lastnameIsEmpty&&!nationalIdentityIsEmpty&&!dateOfBirthIsEmpty&&nationalIdentityisValid&&!nationalIdentityIsUsed&&!passwordIsEmpty&&passwordsSame&&!eMailIsUsed&&!eMailIsEmpty&&mailIsValid) {		
